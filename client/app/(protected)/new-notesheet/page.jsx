...
const NewNotesheetForm = () => {
  ...
  return (
    <form
      ...
      className="flex items-center justify-start gap-10 w-full p-4 overflow-auto"
    >
      <div
        onDrop={handleDrop}
        onDragOver={(e) => e.preventDefault()}
        className="overflow-hidden h-full flex flex-col items-center justify-center text-center w-1/3 hover:bg-gray-100 border-gray-400 hover:border-black border-dashed border-[4px] rounded-lg gap-8 transition-all duration-500"
      >
        <h4 className="xl:w-[40rem] w-[25rem] overflow-hidden text-ellipsis">
          {pdfFile ? (
        ...
        {pdfFile ? (
          <button onClick={()=> {setPdfFile(null); setPdfFileUrl(null)}} className="w-[12rem] flex items-center justify-center bg-[#2f2f2f] text-white h-[40px] px-4 rounded-sm hover:bg-[#0e0202] text-[1.5rem]">
            <h4>Reset PDF</h4>
          </button>
        ) : (
          <input
            ...
            className="xl:text-[2rem] text-[1.5rem]"
			required
          />
        )}
      </div>
      <div className="h-5/6 w-1 bg-black"></div>
      <div className="flex flex-col justify-start h-full w-1/2 gap-10 ">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-wrap gap-10">
              {["GenSec", "PIC", "VP Gymkhana", "ARSA", "DrSA"].map(
                (approval) => (
                  <label key={approval} className="flex items-center cursor-pointer">
                    <input
                      type="checkbox"
                      {...register("approvals", {
              ...
              <label key="adean" className="flex items-center">
                <input
                  ...
                  value="adean"
                  className="mr-2 w-7 h-7 border-black border-solid bg-transparent text-[2rem]"
                />
