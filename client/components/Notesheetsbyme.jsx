    ...
    <div className="flex flex-col gap-3">
      {notesheets?.map((notesheet, index) => (
        ...
        >
          ...
          <p className="w-5/12 p-3 rounded-xl">{notesheet?.subject}</p>
          <p className="w-1/12 p-3 rounded-xl">
          {formatDate(notesheet?.raisedAt)}
          </p>
          <p className="w-1/12 p-3 rounded-xl">₹{notesheet?.amount}</p>
          {(params.get('type') === "to-approve" || params.get('type') === "approved-by-me")  ? <p className="w-2/12 p-3 rounded-xl text-center">₹{notesheet?.amount}</p> : null}
